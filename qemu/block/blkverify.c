                                  ...
                                  void *buf)
{
    ...
    for (i = 0; i < src->niov; i++) {
        ...
        if (last_end && last_end > cur->iov_base) {
            rewind = (char *)last_end - cur->iov_base;
        }

        sortelems[i].dest_base = (char *)buf - rewind;
        (char *)buf += cur->iov_len - MIN(rewind, cur->iov_len);
        last_end = MAX((char *)cur->iov_base + cur->iov_len, last_end);
    }
