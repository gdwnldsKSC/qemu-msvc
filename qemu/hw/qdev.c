...
int qdev_device_help(QemuOpts *opts)
{
    ...
    for (prop = info->props; prop && prop->name; prop++) {
        ...
        error_printf("%s.%s=%s\n", info->name, prop->name,
                     prop->info->legacy_name ? prop->info->legacy_name : prop->info->name);
    }
...
void qdev_free(DeviceState *dev)
{
    ...
    if (dev->ref != 0) {
        qerror_report(QERR_DEVICE_IN_USE, dev->id ? dev->id :"");
    }
                       ...
                       Error **errp)
{
    ...
    if (prop == NULL) {
        error_set(errp, QERR_PROPERTY_NOT_FOUND, dev->id ? dev->id :"", name);
        return;
    }
