...
static uint32_t musb_readb(void *opaque, target_phys_addr_t addr)
{
	...
	case MUSB_HDRC_EP_IDX + 0x1:
	case MUSB_HDRC_EP_IDX + 0x2:
	case MUSB_HDRC_EP_IDX + 0x3:
	case MUSB_HDRC_EP_IDX + 0x4:
	case MUSB_HDRC_EP_IDX + 0x5:
	case MUSB_HDRC_EP_IDX + 0x6:
	case MUSB_HDRC_EP_IDX + 0x7:
	case MUSB_HDRC_EP_IDX + 0x8:
	case MUSB_HDRC_EP_IDX + 0x9:
	case MUSB_HDRC_EP_IDX + 0xa:
	case MUSB_HDRC_EP_IDX + 0xb:
	case MUSB_HDRC_EP_IDX + 0xc:
	case MUSB_HDRC_EP_IDX + 0xd:
	case MUSB_HDRC_EP_IDX + 0xe:
	case MUSB_HDRC_EP_IDX + 0xf:
#endif
	...
	default:
		{
            ...
            else if (addr >= MUSB_HDRC_FIFO && (MUSB_HDRC_FIFO + 0x3f)) {
                ep = ((addr - MUSB_HDRC_FIFO) >> 2) & 0xf;
                musb_write_fifo(s->ep + ep, value & 0xff);
                musb_write_fifo(s->ep + ep, (value >> 8) & 0xff);
            }
