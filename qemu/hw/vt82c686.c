...
static void superio_ioport_writeb(void *opaque, uint32_t addr, uint32_t data)
{
    ...
    if (addr == 0x3f0) {
        ...
        default:
#else
        if ((superio_conf->index >= 0x00 && superio_conf->index <= 0xdf)
            || superio_conf->index == 0xe4
            || superio_conf->index == 0xe5
            || (superio_conf->index >= 0xe9 && superio_conf->index <= 0xed)
            || superio_conf->index == 0xf3
            || superio_conf->index == 0xf5
            || superio_conf->index == 0xf7
            || (superio_conf->index >= 0xf9 && superio_conf->index <= 0xfb)
            || (superio_conf->index >= 0xfd && superio_conf->index <= 0xff))
        {
            can_write = 0;
        }
        else {
